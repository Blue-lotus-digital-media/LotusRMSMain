...
namespace LotusRMSweb.Helper
{
    ...
    public class ActiveClassTagHelper : TagHelper
    {
        ...
        private void MakeActive(TagHelperOutput output)
        {
            ...
            if (classAttr == null)
            {
                classAttr = new TagHelperAttribute("class", "active-route");
                output.Attributes.Add(classAttr);
            }
            else if (classAttr.Value == null || classAttr.Value.ToString().IndexOf("active-route") < 0)
            {
                output.Attributes.SetAttribute("class", classAttr.Value == null
                    ? "active-route"
                    : classAttr.Value.ToString() + " active-route");
            }
        }
    }
}